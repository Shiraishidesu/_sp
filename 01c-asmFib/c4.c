...
int run(int *pc, int *bp, int *sp) { // 虛擬機 => pc: 程式計數器, sp: 堆疊暫存器, bp: 框架暫存器
  ...
  while (1) {
    i = *pc++; ++cycle;
    if (debug) {
      char OpList[] = "LLA ,IMM ,JMP ,JSR ,BZ  ,BNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PSH ,"
         "OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,"
         "OPEN,READ,CLOS,PRTF,MALC,FREE,MSET,MCMP,EXIT,";
         printf("%d> %.4s", cycle,
          &OpList[i * 5]);
...
int main() // 主程式
{
  int *pc, *bp, *sp, poolsz, *t, *fib, *loc, *loc2;
...
// while(i<=n){
  ...
  *e++ = LI; 
  *e++ = LE;
  *e++ = BZ; loc = e; *e++ = 0;//若條件不滿足則跳出至loc = e，需有e++，但不一定要 = 0;
//7:     r = r * 2;
  *e++ = LLA; *e++ = -1;
  *e++ = PSH;
  *e++ = LLA; *e++ = -1;
